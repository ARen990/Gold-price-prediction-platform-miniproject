...
def main(out_json_path: str | None = None):
    ...
    end_date_env = "2025-10-03"
    parquet_out = get_env("OUT_PARQUET", "/opt/shared-data/processed/long/latest.parquet")
    ...
    if 'index' in pdf_all.columns:
        pdf_all = pdf_all.rename(columns={'index': 'datetime'})
    elif 'Date' in pdf_all.columns:
        pdf_all = pdf_all.rename(columns={'Date': 'datetime'})

    # แปลงเป็น datetime
    pdf_all["datetime"] = pd.to_datetime(pdf_all["datetime"])

    # 3) ทำความสะอาดข้อมูล
    # 3.1 กรองตาม end_date
    if end_date_env:
    ...
    # ลด threshold หรือใช้ percentage-based threshold
    miss_th = int(get_env("MISSING_THRESHOLD", "10"))  # ลดจาก 20 เป็น 10

    # หรือใช้ percentage
    missing_ratio = pdf_all.isnull().sum(axis=1) / len(pdf_all.columns)
    pdf_all = pdf_all[missing_ratio <= miss_th_percent]  # ยอมรับ missing ได้ไม่เกิน 30%
