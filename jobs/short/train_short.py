...
MODEL_DIR = Path("/opt/artifacts/model/short")
...
def download_enhanced_data(symbols, period="730d", interval="1h", end_date=None):
    print(f">> Downloading {period} of financial data with technical indicators...")
    frames = []
...
def feature_selection(X_train, y_train, X_test, n_features=20):
...
def train_sarimax(X_train, y_train, X_test, targets, predictions, models):
    ...
    for target_col in targets:
        ...
        try:
            ...
            print(f"  [SUCCESS] SARIMAX forecast generated for {target_col}")
...
def train_bvar(X_train, y_train, X_test, targets, predictions, models):
    ...
    for target_col in targets:
        ...
        try:
            ...
            print(f"  [SUCCESS] BVAR forecast generated for {target_col}")
...
def train_enhanced_lstm(X_train, y_train, X_test, targets, predictions, models):
    ...
    time_steps = 20  # Increased time steps
    ...
    print(f"  [SUCCESS] LSTM forecast generated")
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(MODEL_DIR / 'short_lstm_training_history.png')
...
def train_prophet(X_train, y_train, X_test, targets, predictions, models):
    ...
    for target_col in targets:
        ...
        print(f"  [SUCCESS] Prophet forecast generated for {target_col}")
...
def save_models(models, targets, scaler_X=None, scaler_y=None, selected_features=None):
    ...
    with open(MODEL_DIR / 'short_model_metadata.pkl', 'wb') as f:
        pickle.dump(metadata, f)
    
    for target_col in targets:
        if target_col in models['SARIMAX']:
            with open(MODEL_DIR / f'short_sarimax_{target_col}.pkl', 'wb') as f:
                pickle.dump(models['SARIMAX'][target_col], f)
        if target_col in models['Prophet']:
            with open(MODEL_DIR / f'short_prophet_{target_col}.pkl', 'wb') as f:
                pickle.dump(models['Prophet'][target_col], f)
    
    for target_col in models['Bayesian VAR']:
        with open(MODEL_DIR / f'short_bvar_{target_col}.pkl', 'wb') as f:
            pickle.dump(models['Bayesian VAR'][target_col], f)
    
    if models['LSTM'] is not None:
        models['LSTM'].save(MODEL_DIR / 'short_lstm.keras')
    
    if scaler_X is not None and scaler_y is not None:
        with open(MODEL_DIR / 'short_scaler_X.pkl', 'wb') as f:
            pickle.dump(scaler_X, f)
        with open(MODEL_DIR / 'short_scaler_y.pkl', 'wb') as f:
...
def walk_forward_backtest(df, targets, base_features,
                        train_min_days=300,   # Reduced for hourly data
                        test_days=60,         # Reduced for hourly data  
                        step_days=60,         # Reduced for hourly data
    ...
    while True:
        ...
        try:
            X_train_sel, X_test_sel, selected = feature_selection(X_train, y_train, X_test, n_features=20)
        except Exception as e:
...
def main():
    ...
    # Download enhanced data with hourly frequency (pass end date)
    df = download_enhanced_data(symbols, period="730d", interval="1h", end_date=end_dt)
    ...
    if not USE_WALK_FORWARD:
        ...
        X_train, X_test, selected_features = feature_selection(
            X_train_original, y_train, X_test_original, n_features=20
        )
        ...
        #                 evaluate(y_test[target_col], pred_df[target_col], f"{model_name}-{target_col}")
        ...
        metrics_df, all_preds, all_ytest, summary_results = walk_forward_backtest(
            df, targets, features,
            train_min_days=300, 
            test_days=60,      
            step_days=60,      
        ...
        metrics_df.to_csv(MODEL_DIR / 'short_backtest_metrics.csv', index=False)
        print(f"\nBacktesting completed with {len(metrics_df)} folds")
        print(f"Results saved to {MODEL_DIR}short_backtest_metrics.csv")
