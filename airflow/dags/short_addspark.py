...
SPARK_APP = "/opt/spark-apps/etl_short.py"           # จาก compose: ./spark_jobs:/opt/spark-apps
...
with DAG(
    ...
    spark_etl = SparkSubmitOperator(
        ...
        "OUT_PARQUET": "/opt/shared-data/processed/short/latest.parquet",
    ...
    task_train = BashOperator(
        task_id="task_train",
        bash_command=f"python {JOBS_DIR}/short/train_short.py",
    )
    task_predict = BashOperator(
        task_id="task_predict",
        bash_command=f"python {JOBS_DIR}/short/predict_short.py",
    )
